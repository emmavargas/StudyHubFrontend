...
async function createTopicCard(title, description, bibliography){
    ...
    try{
        const response = await fetch (`http://65.21.56.202:3000/user/courses/${idCourse}/topics`, {
            method: 'POST',
            headers:{
...
async function getDataTopic(idTopic){
    let dataTopic={};
    try{
        const response = await fetch(`http://65.21.56.202:3000/user/courses/${idCourse}/topics/${idTopic}`, {
            method:'GET',
            credentials:'include'
        });
        if(!response.ok){
